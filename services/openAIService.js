...
async function generateImage(ideaId, prompt, references = []) {
  ...
  try {
    ...
    try {
      ...
      let status = 'failed';
      if (errorMsg.includes('safety_violations') || errorMsg.includes('rejected by the safety system')) {
        status = 'safety_violation';
      }
      
      const failureUpdateParams = [status, errorMsg.substring(0, 255), ideaId];
      // Validate parameters
      if (failureUpdateParams.some(p => p === undefined)) {
        ...
        console.log(`Updated database status to ${status} for idea ${ideaId}`);
      }
    } catch (dbError) {
